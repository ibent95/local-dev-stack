#!/usr/bin/env bash
# Ensure Postgres has the LDS Analytics database/user. Reuses postgres-init by
# injecting the analytics spec into POSTGRES_INIT_SPECS for this run.
# Idempotent; auto-run by `lds up` for analytics/all.
set -euo pipefail
...
db="${ANALYTICS_POSTGRES_DB:-lds_analytics}"
u="${ANALYTICS_POSTGRES_USER:-app}"
p="${ANALYTICS_POSTGRES_PASSWORD:-app}"
spec="${db}:${u}:${p}"
...
schema="$ROOT/configs/analytics/api/schema.sql"
if [ -f "$schema" ]; then
  echo "Applying Analytics DB schema to '$db'…"
