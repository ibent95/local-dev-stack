...
- Services are gated by profiles: `proxy`, `php`, `mysql`, `postgres`, `mongo`,
  `redis`, `memcached`, `kafka`, `phpcacheadmin`, `dbgate`, `soketi`,
  `centrifugo`, `emqx`, `all`. Each one is described in detail in
...
- `lds up` with no profiles starts every profile whose `LDS_ENABLE_<PROFILE>`
  ...
  (`soketi`, `centrifugo`, `emqx`) are off by default — start one with
